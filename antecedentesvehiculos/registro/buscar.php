...
if ($Estado == 1) {
	$ConsultaAntecedentes = "CodVehiculo IN(SELECT CodVehiculo FROM antecedentevehiculo WHERE Confirmado=0 and Activo=1 GROUP BY codvehiculo) and";
} else {
...
$su = $vehiculo->mostrarTodoRegistro("$ConsultaAntecedentes CodMarca LIKE '$CodMarca' and CodServicioTransporte LIKE '$CodServicioTransporte'  and Placa LIKE '%$Placa%' and CodChofer IN (SELECT CodChofer FROM chofer WHERE Ci LIKE '%$Ci%')", 1, "Placa", 1);
//print_r($di);
...
<table class="table table-hover table-bordered table-striped table-table-condensed">
	...
	<tbody>
		...
		?>
			<tr>
				<td colspan="5">
					No existen registros de los criterios buscados
				</td>
			</tr>
		<?php
		}
