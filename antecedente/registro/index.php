...
$CodChofer=$_GET['CodChofer'];



include_once("../../class/chofer.php");
$chofer=new chofer;
$ch=$chofer->mostrarTodoRegistro("CodChofer=".$CodChofer,1,"Nombres");
$ch=array_shift($ch);
...
$d=$vehiculo->mostrarTodoRegistro("CodChofer=$CodChofer",1,"");
$d=array_shift($d);
...
$titulo="Registro de Antecedentes de Chofer/Taxista";
$folder="../../";
...
$(document).ready(function(){
		...
		swal("¿Esta Seguro de Guardar Antecedente?",{
			...
			switch (value) {
				case "ok":{
					...
					var CodChofer=<?=$CodChofer;?>;
					var FechaAntecedente=$("[name=FechaAntecedente]").val();
					var DetalleAntecedente=$("[name=DetalleAntecedente]").val();
					var CodAntecedenteChofer=$("[name=CodAntecedenteChofer]").val();


					$.post("guardar.php",{"CodChofer":CodChofer,FechaAntecedente:FechaAntecedente,DetalleAntecedente:DetalleAntecedente,CodAntecedenteChofer:CodAntecedenteChofer},function(data){
						listarAntecedentes();
						listarAntecedentesPendientes();
					});
				}break;
	...
	$(document).on("click","#verificar",function(e){
		...
		var CodAntecedenteChofer=$(this).attr("rel");
		...
		$("[name=CodAntecedenteChofer]").val(CodAntecedenteChofer);
		// alert(FechaV);
	});
...
function listarAntecedentes(){
	var CodChofer=<?=$CodChofer;?>;
	$.post("listarantecedentes.php",{"CodChofer":CodChofer},function(data){
...
function listarAntecedentesPendientes(){
	var CodChofer=<?=$CodChofer;?>;
	$.post("antecedentespendientes.php",{"CodChofer":CodChofer},function(data){
	...
	<div class="col-lg-6">
		<div class="panel">
			<div class="panel-body">
				...
				<input type="hidden" name="CodAntecedenteChofer" value="">
				<table class="table">
					<tr>
